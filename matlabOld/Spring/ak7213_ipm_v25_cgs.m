...
while dualGap(ipmIter) > ipmTol && ipmIter <= ipmMaxIter
    ...
    %% Compute Sigma, Beta, Mu
    Sigma = (t.^-1).*lambda;
    Beta = getBeta(n,N,l,Sigma);
    Mu = getMu(n,N,l,Sigma,Beta);
    ...
    for k = 1:N
        for j = 1+(k-1)*nj:n
            ...
            %Term 1 - Diagonal
            U(j,k) = U(j,k) + sum((s.^2).*EInv) - Beta(j,k)*((s'*EInv).^2);

            %Term 2 - Rank 1 Matrix (v*v')
            V(j,k) = V(j,k) + s'*EInv - Beta(j,k)*sum(EInv)*(s'*EInv);
        end
    end
    ...
    for k = 1:N
        ...
        for j = 1:n
            sel = 1+(j-1)*l:j*l;
            EInv = Sigma(n+1+sel,k).^-1;
            u(j) = sum(s.*EInv.*raHat(sel,k)) - Beta(j,k)*(s'*EInv)*(EInv'*raHat(sel,k));
            v(j) = s'*EInv - Beta(j,k)*sum(EInv)*(s'*EInv);
            Rho = Rho + sum(EInv.*raHat(sel,k)) - Beta(j,k)*sum(EInv)*(EInv'*raHat(sel,k));
        end
        PRHS = PRHS + cropVector(u,nj,k) - Mu(k)*Rho*cropVector(v,nj,k);
    end
    ...
    for k = 1:N
        ...
        for j = 1:n
            sel = 1+(j-1)*l:j*l;
            EInv = Sigma(n+1+sel,k).^-1;
            u(j) = sum((s.^2).*EInv) - Beta(j,k)*((s'*EInv).^2);
            v(j) = s'*EInv - Beta(j,k)*sum(EInv)*(s'*EInv);
        end
        PRHS = PRHS + (cropVector(u,nj,k).*sum(rx(:,k:N),2)) - Mu(k)*(v'*sum(rx(:,k:N),2))*cropVector(v,nj,k);
    end
    ...
    for k = 1:N
        ...
        for j = 1:n
            sel = 1+(j-1)*l:j*l;
            EInv = Sigma(n+1+sel,k).^-1;
            u(sel) = (EInv.*y(sel)) - Beta(j,k)*EInv*(EInv'*y(sel));
            v(sel) = EInv - Beta(j,k)*EInv*(sum(EInv));            
        end
        Rho = sum(u);
        affDa(:,k) = u - Mu(k)*Rho*v;
    end
    ...
    for k = 1:N
        ...
        CRHS = CRHS + cropVector(u,nj,k) - Mu(k)*Rho*cropVector(v,nj,k);
    end
    ...
    for k = 1:N
        ...
        CRHS = CRHS + (cropVector(u,nj,k).*sum(rx(:,k:N),2)) - Mu(k)*(v'*sum(rx(:,k:N),2))*cropVector(v,nj,k);
    end
...
%% Utility Functions
function Mu = getMu(n,N,l,Sigma,Beta)
    Mu = zeros(1,N);
    for k = 1:N
        v = zeros(n*l,1);
        for j = 1:n
        ...
        Mu(k) = Sigma(n+1,k)/(1+Sigma(n+1,k)*sum(v));
    end
end

function Beta = getBeta(n,N,l,Sigma)
    Beta = zeros(n,N);
    for k = 1:N
        for j = 1:n
            sel = 1+(j-1)*l:j*l;
            EInv = Sigma(n+1+sel,k).^-1;
            Beta(j,k) = Sigma(j,k)/(1 + Sigma(j,k)*sum(EInv));
        end
    end
end
